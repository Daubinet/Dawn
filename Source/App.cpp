...
void App::initialize()
{
	...
	// since this is basically a CEGUI app, we can use the ST_GENERIC scene manager for now; in a later article we'll see how to change this
	gSceneManager = ogre->createSceneManager(Ogre::ST_GENERIC);
	camera = gSceneManager->createCamera("camera");
    Ogre::Viewport* vp = window->addViewport(camera);
    vp->setBackgroundColour(Ogre::ColourValue(0,0,0));

	// most examples get the viewport size to calculate this; for now, we'll just set it to 4:3 the easy way
	...
	game.initialize(camera, handler);
}
void App::destroyWindow()
{
	if(gSceneManager != NULL){
		gSceneManager->destroyAllCameras();
		gSceneManager->destroyAllEntities();
		gSceneManager->destroyAllParticleSystems();
		gSceneManager->destroyAllLights();
		gSceneManager->destroyAllManualObjects();
		gSceneManager->destroyAllAnimations();
		gSceneManager->destroyAllMovableObjects();
	}
